...
def game_solver_GBFS(current_config, goal_config, hash_row, hash_col):
    ...
    frontier.put((heuristic, states, initial))
    visited.add(states)
    ...
    while not frontier.empty():
        ...
        for i in range(len(available)):
            ...
            if next_state not in visited:
                ...
                h = manhattan_heuristic(next_config, goal_cells)
                frontier.put((h, next_state, GameSnapshot(new_row, new_col, current.moves + 1, next_config)))
...
solution, expanded_nodes = game_solver_GBFS(current_board_configuration, goal_board_configuration, hash_pos_row, hash_pos_col)
